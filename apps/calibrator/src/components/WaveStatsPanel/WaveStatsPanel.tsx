  ...
  return (
    <div style={{ width: '25%', minWidth: 180, padding: 12, display: 'flex', flexDirection: 'column', gap: 10, borderRight: '1px solid #2c2c3e', overflow: 'auto' }}>
      <div style={{ background: '#2c2c3e', borderRadius: 4, padding: 10 }}>
        <div style={{ fontSize: 11, color: '#666', marginBottom: 6 }}>Wave Info</div>
        <div style={{ fontSize: 13, color: '#aaa' }}>{enemyCount} inimigo{enemyCount !== 1 ? 's' : ''}</div>
        <div style={{ fontSize: 13, color: '#aaa' }}>Delay: {wave.delay.toFixed(1)}s</div>
      </div>
      <ScoreCard grid={wave.grid} delay={wave.delay} />
      <div style={{ fontSize: 11, color: '#555', marginTop: 4 }}>Level Params</div>
